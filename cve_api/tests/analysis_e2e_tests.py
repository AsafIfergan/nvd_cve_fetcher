import unittest
from cve_api.cve_analyzer import get_jsons_analysis
...
class TestAnalyzeE2E(unittest.TestCase):
    ...
    def test_analysis(self):
        ...
        total_average, severities, top_affected_packages = get_jsons_analysis(
            self.test_jsons_path
        )

        expected_total_average = 6.59004149377592
        expected_severities = {"medium": 156, "high": 63, "critical": 18, "low": 4}
        expected_top_affected_packages = [
            ("myq-solution_print_server", 48),
            ("ajaysharma_cups_easy", 41),
            ("apple_macos", 32),
            ("appleple_a-blog_cms", 25),
            ("linecorp_line", 23),
        ]
